...
- Added a tenant detail surface in the desktop customer workspace with dedicated tenant metrics and route-style follow-up actions.
- Added visual workspace signal cards to the desktop overview so the home screen reads more like a product dashboard than a text-heavy utility shell.

### Changed

- Refined the connected desktop flow so tenant focus can branch more naturally into operations and reporting.
